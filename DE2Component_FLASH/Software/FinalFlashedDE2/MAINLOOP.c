/*	This file contains functions called from the main loop.
 *
 *	This file is from the DE2 demonstration project with a few exceptions.
 *	The code has been modified in the following ways:
 *		- Class requests for HID have been added (will be called from DeviceRequest_Handler function)
 *		- Removed some of the Vendor requests that are not handled
 *		- config_endpoints function modified to set endpoints and hardware to desired settings
 *		- Some function names changed to be more descriptive of function
 */
...
code CHAR * _NAME_USB_REQUEST_RECIPIENT[] = { "Device", "Interface",
		"Endpoint(0)", "Other" };

code CHAR * _NAME_USB_REQUEST_TYPE[] = { "Standard", "Class", "Vendor",
		"Reserved" };

code CHAR * _NAME_USB_STANDARD_REQUEST[] = { "GET_STATUS", "CLEAR_FEATURE",
		"RESERVED", "SET_FEATURE", "RESERVED", "SET_ADDRESS", "GET_DESCRIPTOR",
...
code CHAR * _NAME_USB_CLASS_REQUEST[] = { "ML_Reserved", "Get_Report",
		"Get_Idle", "Get_Protocol", "ML_Reserved", "ML_Reserved", "ML_Reserved",
		"ML_Reserved", "ML_Reserved", "Set_Report", "Set_Idle", "Set_Protocol" };

//*************************************************************************
// Vendor Device Request
//*************************************************************************

#define MAX_VENDOR_REQUEST    0x00
...
void SetupToken_Handler(void) {
	...
	if (j == sizeof(DEVICE_REQUEST)) {
		/* //For debugging prints components of packet
